...
import { sAdminTeamTextApi } from "../../api";
...
class TeamText extends Component {
  ...
  componentDidMount() {
    ...
    sAdminTeamTextApi
      .sAdminGetTeamTextDetails(formattedToken)
      .then((response) => {
        this.setState({
  ...
  handleSubmit(e) {
    ...
    sAdminTeamTextApi
      .sAdminUpdateTeamTextDetails(data, formattedToken)
      .then((response) => {
        this.setState({
          submitting: false,
          errorStatus: null,
    ...
    return (
      <>
        <CRow>
          <CCol xs="12">
            <CCard>
              <CCardHeader>Details of "Team Text"</CCardHeader>
              <CCardBody>
                {submitted && errorStatus === 400 && (
                  <span className="validation-error mt-2">
                    Updating Team Text Failed
                  </span>
                )}
...
export default connect(mapStateToProps)(TeamText);
