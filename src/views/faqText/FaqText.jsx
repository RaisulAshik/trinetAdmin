...
import { sAdminFaqTextApi } from "../../api";
...
class FaqText extends Component {
  ...
  componentDidMount() {
    ...
    sAdminFaqTextApi
      .sAdminGetFaqTextDetails(formattedToken)
      .then((response) => {
        this.setState({
  ...
  handleSubmit(e) {
    ...
    sAdminFaqTextApi
      .sAdminUpdateFaqTextDetails(data, formattedToken)
      .then((response) => {
        this.setState({
          submitting: false,
          errorStatus: null,
    ...
    return (
      <>
        <CRow>
          <CCol xs="12">
            <CCard>
              <CCardHeader>Details of "FAQ Text"</CCardHeader>
              <CCardBody>
                {submitted && errorStatus === 400 && (
                  <span className="validation-error mt-2">
                    Updating FAQ Text Failed
                  </span>
                )}
...
export default connect(mapStateToProps)(FaqText);
