...
import { sAdminAwardTextApi } from "../../api";
...
class AwardText extends Component {
  ...
  componentDidMount() {
    ...
    sAdminAwardTextApi
      .sAdminGetAwardTextDetails(formattedToken)
      .then((response) => {
        this.setState({
  ...
  handleSubmit(e) {
    ...
    sAdminAwardTextApi
      .sAdminUpdateAwardTextDetails(data, formattedToken)
      .then((response) => {
        this.setState({
          submitting: false,
          errorStatus: null,
    ...
    return (
      <>
        <CRow>
          <CCol xs="12">
            <CCard>
              <CCardHeader>Details of "Award Text"</CCardHeader>
              <CCardBody>
                {submitted && errorStatus === 400 && (
                  <span className="validation-error mt-2">
                    Updating Award Text Failed
                  </span>
                )}
...
export default connect(mapStateToProps)(AwardText);
