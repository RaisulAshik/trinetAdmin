...
import { sAdminServiceTextApi } from "../../api";

import { loaderUrl } from "src/constants";

import "./ServiceText.css";

class ServiceText extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    ...
    sAdminServiceTextApi
      .sAdminGetServiceTextDetails(formattedToken)
      .then((response) => {
        this.setState({
  ...
  handleSubmit(e) {
    ...
    sAdminServiceTextApi
      .sAdminUpdateServiceTextDetails(data, formattedToken)
      .then((response) => {
        this.setState({
          submitting: false,
          errorStatus: null,
    ...
    return (
      <>
        <CRow>
          <CCol xs="12">
            <CCard>
              <CCardHeader>Details of "Service Text"</CCardHeader>
              <CCardBody>
                {submitted && errorStatus === 400 && (
                  <span className="validation-error mt-2">
                    Updating Service Text Failed
                  </span>
                )}
...
export default connect(mapStateToProps)(ServiceText);
